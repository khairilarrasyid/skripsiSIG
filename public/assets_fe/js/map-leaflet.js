$(document).ready(function ($) {
    ...
    if ($("#" + mapId).length) {
        ...
        if (mapElement.attr("data-ts-display-additional-info")) {
            ...
            for (var i = 0; i < displayAdditionalInfoTemp.length; i++) {
                displayAdditionalInfo.push(
                    displayAdditionalInfoTemp[i].split("_")
                );
            }
        }
    ...
    $("#search-destinasi").on("submit", function (e) {
        e.preventDefault();

        var keyword = $("#keyword").val();
        var category = $("#category").val();
        $.ajax({
            url: "http://127.0.0.1:8000/fetch-peta",
            dataType: "json",
            method: "GET",
            cache: false,
            data: {
            ...
            markerContent.innerHTML =
                ...
                (loadedMarkersData[i]["marker_image"] !== undefined
                    ? '<div class="ts-marker__image ts-black-gradient" style="background-image: url(' +
                      ...
                      ')"></div>'
                    : '<div class="ts-marker__image ts-black-gradient" style="background-image: url(assets_fe/img/marker-default-img.png)"></div>') +
                "</a>" +
                "</div>";
    ...
    function createInfoBoxHTML(parameters) {
        ...
        infoboxHtml.innerHTML =
            ...
            loadedMarkersData[i]["id"] +
            '">' +
            '<img src="/assets_fe/img/infobox-close.svg" class="ts-close">' +
            '<a href="' +
        ...
        for (i = 0; i < visibleMarkersOnMap.length; i++) {
            ...
            resultsHtml.push(
                '<div class="ts-result-link" data-ts-id="' +
                    ...
                    '">' +
                    '<span class="ts-center-marker"><img src="assets_fe/img/result-center.svg"></span>' +
                    '<a href="' +
    ...
    if ($("#" + simpleMapId).length) {
        ...
        var markerDrag = parseInt(
            mapElement.attr("data-ts-map-marker-drag"),
            10
        );
        ...
        var icon = L.icon({
            iconUrl: "http://127.0.0.1:8000/assets_fe/img/marker-small.png",
            iconSize: [22, 29],
            iconAnchor: [11, 29],
        });
