...
@section('content')
    <form id="create-form" action="{{ route('destinations.update', $destination->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                    ...
                    <div class="avatar avatar-image rounded" style="height: 70px; width: 100px">
                        <div id="imagePreview">
                            @if ($destination->image)
                                <img src="{{ Storage::url('destinations/' . $destination->image) }}" alt="Preview" style="width: 100%; height: 70px;">
                            @endif
                        </div>
                    </div>
                    <div class="m-l-15">
        ...
        <div class="tab-content m-t-15">
            <div class="tab-pane fade show active" id="product-edit-basic">
                <div class="card">
                    <div class="card-body">
                        <div class="form-group">
                            <label class="font-weight-semibold" for="nama">Nama Destinasi</label>
                            <input type="text" class="form-control @error('nama') is-invalid @enderror" id="nama"
                                name="nama" value="{{ old('nama', $destination->name) }}" required>
                            @error('nama')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                        ...
                        <div class="form-group">
                            <label class="font-weight-semibold" for="kategori">Kategori</label>
                            <select class="custom-select @error('kategori') is-invalid @enderror" id="kategori"
                                name="kategori" required>
                                <option value="">- Pilih Kategori -</option>
                                @foreach ($categories as $category)
                                    <option
                                        {{ old('kategori', $destination->category_id) == $category->id ? 'selected' : '' }}
                        ...
                        <div class="form-group">
                            ...
                            $start = null;
                            $end = null;
                            $exploded = explode(' - ', $destination->opening_hours);
                            if (count($exploded) == 2) {
                                $start = $exploded[0];
                                $end = $exploded[1];
                            }
                        ...
                        <div class="form-group">
                            ...
                            <input type="number" class="form-control" id="harga_tiket" name="harga_tiket"
                                value="{{ old('harga_tiket', $destination->ticket_price) }}">
                        </div>

                        <div class="form-group">
                            <label class="font-weight-semibold" for="alamat">Alamat</label>
                            <input type="text" class="form-control @error('alamat') is-invalid @enderror" id="alamat"
                                name="alamat" value="{{ old('alamat', $destination->address) }}" required>
                            @error('alamat')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
            ...
            <div class="tab-pane fade" id="product-edit-description">
                <div class="card">
                    <div class="card-body">
                        <div id="deskripsi">
                            {!! old('deskripsi', $destination->description) !!}
                        </div>
                        <input type="hidden" id="quill_html" name="deskripsi"
                            value="{{ old('deskripsi', $destination->description) }}">
                    </div>
                </div>
            </div>
        </div>
